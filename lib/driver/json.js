...
class JSONObject extends obj.UserObject {
  ...
  Dot(idx) {
    ...
    throw new obj.TypeMismatch("expect String but get %s",typeof idx);
  }
  ...
  GetAttribute(x) {
    if(x instanceof obj.String) {
      if(x.value == "size") {
        return new obj.Number(this._obj.length);
      } else if(x.value == "empty") {
        return new obj.Boolean(this._obj.length == 0);
      } else if(x.value == "type") {
        return "JSONObject";
      }
      throw new obj.NoAttribute(util.format("no such attribute %s found in type JSONObject",x.value));
    }
    throw new obj.TypeMismatch(util.format("expect String but get %s",typeof x));
  }
...
class JSONArray extends obj.UserObject {
  ...
  Index(idx) {
    ...
    throw new obj.TypeMismatch(util.format("expect Number but get %s",typeof x));
  }
...
class JSONDriver extends obj.UserObject {
  Invoke(e,arg) {
    ...
    if(arg[0] instanceof obj.String) {
      return _New(JSON.parse(fs.readFileSync(arg[0].value)));
    }
    console.log(arg[0]);
    throw new obj.TypeMismatch(
      util.format("expect String as 1st argument for function JSON but get type %s",typeof arg[0]));
  }
};

module.exports = {
  FactoryName   : "JSON",
  FactoryMethod : JSONDriver
};
