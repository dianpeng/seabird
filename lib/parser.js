...
class ParserError extends excep.FatalException {
...
class Parser {
  ...
  _GetCodeSnippet() {
    let cursor = this.lexer.GetTokenPosition();
    const half = lexer.Position.MinCodeSnippetSize / 2;

    let start = cursor < half ? 0 : cursor - half;
    let end   = start;
    if( end + half > this.lexer.source.length )
      end = this.lexer.source.length;
    else
      end += half;
    let src = this.lexer.source.substr(start,end-start);
    return lexer.Position.FormatCodeSnippet(src);
  }

  _Error(msg) {
    throw new ParserError(util.format("Parser failed at (line:%d,position:%d) around code snippet %s with error:%s\n%s",
                                      this.lexer.line,
                                      this.lexer.ccount,
                                      this._GetCodeSnippet(),
                                      msg,
                                      new Error().stack));
  }
        ...
        case lexer.Token.Dot:
          switch(this.lexer.Next().token) {
            ...
            case lexer.Token.Wildcard:
              ...
              break;
            default:
              this._Error(util.format("unrecognized token %s after \".\"",this.lexer.GetTokenName()));
              break;
          }
  ...
  _ParseProgram() {
    let defines = [];
    let start_pos = this.lexer.GetTokenPosition();
    while(this.lexer.GetToken() == lexer.Token.Let) {
      defines.push(this._ParseLet());
    }
    ...
    // eat the optional semicolon to make it pragmatic
    if(this.lexer.GetToken() == lexer.Token.Semicolon) {
      this.lexer.Next();
    }
