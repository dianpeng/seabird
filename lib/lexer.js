...
class Position {
  ...
  static get MinCodeSnippetSize() { return 32; }
  static get CodeSeparatorL    () { return ">>>>>>>>>>>>>>>>>>>>>>>>>"; }
  static get CodeSeparatorR    () { return "<<<<<<<<<<<<<<<<<<<<<<<<<"; }
  static FormatCodeSnippet     (src) {
    return "\n" + Position.CodeSeparatorL + "\n" + src + "\n" + Position.CodeSeparatorR + "\n";
  }
  ...
  GetCodeSnippet() {
    ...
    if(length < Position.MinCodeSnippetSize) {
      const half = Position.MinCodeSnippetSize / 2;
      // this code snippet is too small, then just try to enlarge it
      // to at least 5 characters
      if(s >= half) s -=half; else s = 0;
      if((this.source.length - e) <=half) e = this.source.length
      else e += half;
    }
    return this.source.substr(s,e-s);
  }
};
...
class Lexer {
  ...
  _LexString() {
    ...
    this.ccount += this.lexeme.length;
    return this.lexeme;
  }
