...
class Eval {
  ...
  _PopulateExceptionMessage(pos,msg) {
    ...
    return util.format("Evaluation error,around code snippet %s evaluation error occur: %s\nFrame:\n%s\n%s",
                       lexer.Position.FormatCodeSnippet(pos.GetCodeSnippet()),
                       msg,
                       stk.join("\n"),
                       jsstk);
  }
  ...
  _EvalDot(this_ptr,x) {
    if(obj.IsHeapType(this_ptr)) {
      ...
      if(comp instanceof ast.Variable) {
        ...
        excep.OnBug("unknown dot operator RHS,possible bug in parser",x);
      }
    }
  ...
  _EvalLambdaCall(pos,lambda,frame,narg) {
    ...
    if(narg != lambda.code.argument.length) {
      this._Fatal(pos,util.format("cannot call function with argument number %d but expect %d",
                                  narg,lambda.code.argument.length));
    }
  ...
  _EvalPrefixComponent(this_ptr,x) {
    ...
    switch(x.type) {
      ...
      default:
        excep.OnBug("unknown prefix component in Prefix expression, possible bug in parser",x);
    }
  }
  ...
  _EvalAst(x) {
    if(x instanceof ast.Number) {
      ...
      excep.OnBug("unknown node shows up in program, possible bug in evaluator??",x);
    }
  }
  ...
  _EvalLet(x) {
    for( const let_scope of x ) {
      for( const e of let_scope.vars ) {
