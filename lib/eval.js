...
class Eval {
  constructor(map,dollar) {
    ...
    if(dollar == null) dollar = new ast.Null();
    this.dollar     = dollar;
    this.show_error_trace = false;
  }

  // Constant field
  static get MaxFrameSize() { return 128; }
  ...
  _ToBoolean(pos,v) {
    ...
    if(v instanceof obj.UserObject)   return v.ToBoolean();
    return true;
  }

  _ToString(pos,v) {
    if(obj.IsInheritFrom(v,obj.ResultSet)) {
      this._Error(pos,"result set cannot be converted to string");
    }
    if(v instanceof obj.Null)       return "null";
    if(v instanceof obj.Boolean)    return v.value ? "true" : "false";
    if(v instanceof obj.Number)     return util.format("%d",v.value);
    if(v instanceof obj.UserObject) return v.ToString();

    this._Error(pos,util.format("type %s cannot be converted to String",typeof v));
  }
  ...
  _ApplyComparisonOp(pos,lhs,rhs,prim_op,obj_op,op) {
    if(this._IsBothNumber(lhs,rhs)) {
      ...
      let l = this._ToString(pos,lhs);
      let r = this._ToString(pos,rhs);
      return this._SetThis(prim_op(l,r));
    } else if(obj.IsInheritFrom(lhs,obj.UserObject)) {
      return this._SetThis(obj_op(lhs,rhs));
    } else {
      this._Error(pos,util.format("cannot apply operator %s on type lhs:%s , rhs:%s",
